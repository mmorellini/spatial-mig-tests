# SCRIPT DESCRIPTION -----------------------------------------------------------
# 02_ppml_main_q50.R
# Purpose: Fit main PPML models with pred_q50 as outcome:
#   - Baseline (FE only)
...
## With origin, destination, and year Fixed Effects
base_pois <- fepois(
  pred_q50 ~ 1 |
...
etable(
  ...
  tex  = TRUE,
  replace = TRUE,
  file = here("tables", "ppml_main_q50.tex"),
  extralines = list(
...
save(base_pois, gravity_pois, mst_pois,
     file = here("data_processed", "ppml_main_q50.RData"))
